...
function wpmm_enqueue_scripts( $hook ) {
    ...
    if('wpmm_location' == $post_type ){
        wp_enqueue_script('display-icons', WPMM_URL . 'js/admin-display-icons.js', array( 'jquery' ), '6.1',true )  ;
    }
...
function wpmm_fetch_stores( $map ) {
	...
	$locations = get_posts( apply_filters( 'wpmm_location_query', $query_args ) );
	...
	foreach ( $locations as $location ) {
		...
		$store_marker = WPMM_URL . '/markers/' . get_post_meta( $location->ID, '_wpmm_marker_icon', true ) . '.png';
		...
		$stores[] = apply_filters ( 'wpmm_location_fields', array(
			'id' => $store_id,
...
function wpmm_get_location_features( $post ) {
	//Returns Array of Term Names for "location features"
...
function wpmm_get_initial_marker_location( $hook ) {
	...
	$marker_vars = apply_filters( 'wpmm_marker_vars', array(
		'latitude' => $lat,
		'longitude' => $lng,
		'post_id' => $post_id,
